  ...
  new: (
    <div className="w-9 h-9 rounded-full bg-violet-100 flex items-center justify-center shrink-0">
      <svg className="w-4 h-4 text-violet-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m8 0H8m8 0h2a2 2 0 012 2v6.764A23.859 23.859 0 0112 17.5a23.86 23.86 0 01-10-2.736V8a2 2 0 012-2h2" />
      </svg>
    </div>
  ),
  like: (
          ...
          {notifications.map((n) => (
            <div
              ...
              className={`flex gap-3 p-3 rounded-2xl transition-colors ${
                n.read ? "bg-white" : "bg-white border-l-[3px] border-violet-500"
              }`}
            >
              {typeIcon[n.type]}
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2">
                  <h3 className={`text-xs leading-tight ${n.read ? "font-medium text-gray-700" : "font-bold text-gray-900"}`}>
                    {n.title}
                  </h3>
                  {!n.read && (
